...
else if ($_SERVER['REQUEST_METHOD'] === 'GET')
{
    ...
    foreach ( $unique_array as $session) {
        $logMsg= $_SESSION['uid'] . " - KILLSESSION: " . $session[1] . " on server " . $session[2] . " for user " . $session[0];
        pushToLogFile($logMsg);
        $returnCode=killRemoteSession($session[2], $session[1]);
        //var_dump($returnCode);
        $logMsg= $_SESSION['uid'] . " - KILLSESSION: " . $session[1] . " on server " . $session[2] . " for user " . $session[0] . " - " . $returnCode;
        pushToLogFile($logMsg);
